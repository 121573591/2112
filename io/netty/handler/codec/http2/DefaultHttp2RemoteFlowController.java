...
implements Http2RemoteFlowController {
    ...
    private int minUsableChannelBytes() {
        return Math.max((int)this.ctx.channel().config().getWriteBufferLowWaterMark(), (int)32768);
    }

    private int maxUsableChannelBytes() {
        int channelWritableBytes = (int)Math.min((long)Integer.MAX_VALUE, (long)this.ctx.channel().bytesBeforeUnwritable());
        int usableBytes = channelWritableBytes > 0 ? Math.max((int)channelWritableBytes, (int)this.minUsableChannelBytes()) : 0;
        return Math.min((int)this.connectionState.windowSize(), (int)usableBytes);
    }

    private int writableBytes() {
        return Math.min((int)this.connectionWindowSize(), (int)this.maxUsableChannelBytes());
    }
    ...
    implements Http2StreamVisitor {
        ...
        void windowSize(FlowState state, int initialWindowSize) {
            ...
            try {
                ...
                throw new RuntimeException("Caught unexpected exception from window", (Throwable)((Object)e));
            }
        }
        ...
        void stateCancelled(FlowState state) {
            try {
                ...
                throw new RuntimeException("Caught unexpected exception from checkAllWritabilityChanged", (Throwable)((Object)e));
            }
        }
    ...
    implements StreamByteDistributor.StreamState {
        ...
        int writeAllocatedBytes(int allocated) {
            ...
            try {
                ...
                while (!(this.cancelled || (frame = this.peek()) == null || (maxBytes = Math.min((int)allocated, (int)this.writableWindow())) <= 0 && frame.size() > 0)) {
                    writeOccurred = true;
                    int initialFrameSize = frame.size();
                    try {
                        frame.write(DefaultHttp2RemoteFlowController.this.ctx, Math.max((int)0, (int)maxBytes));
        ...
        private int writableWindow() {
            return Math.min((int)this.window, (int)DefaultHttp2RemoteFlowController.this.connectionWindowSize());
        }
        ...
        private void enqueueFrameWithoutMerge(Http2RemoteFlowController.FlowControlled frame) {
            this.pendingWriteQueue.offer((Object)frame);
        ...
        void cancel(Http2Error error, Throwable cause) {
            ...
            if (frame != null) {
                ...
                } while ((frame = (Http2RemoteFlowController.FlowControlled)this.pendingWriteQueue.poll()) != null);
            }
            DefaultHttp2RemoteFlowController.this.streamByteDistributor.updateStreamableBytes(this);
        ...
        private void decrementFlowControlWindow(int bytes) {
            try {
                ...
                throw new IllegalStateException("Invalid window state when writing frame: " + e.getMessage(), (Throwable)((Object)e));
            }
        }
        ...
        private void writeError(Http2RemoteFlowController.FlowControlled frame, Http2Exception cause) {
            ...
            frame.error(DefaultHttp2RemoteFlowController.this.ctx, (Throwable)((Object)cause));
        }
    }
}
