...
public final class HttpUtil {
    ...
    private static final String COMMA_STRING = String.valueOf((char)',');
    ...
    public static int getContentLength(HttpMessage message, int defaultValue) {
        return (int)Math.min((long)Integer.MAX_VALUE, (long)HttpUtil.getContentLength(message, (long)defaultValue));
    }
    ...
    public static void setTransferEncodingChunked(HttpMessage m, boolean chunked) {
        if (chunked) {
            ...
            ArrayList values = new ArrayList(encodings);
    ...
    public static Charset getCharset(CharSequence contentTypeValue, Charset defaultCharset) {
        ...
        if (contentTypeValue != null && (charsetRaw = HttpUtil.getCharsetAsSequence(contentTypeValue)) != null) {
            ...
            try {
                return Charset.forName((String)charsetRaw.toString());
            }
    ...
    public static long normalizeAndGetContentLength(List<? extends CharSequence> contentLengthFields, boolean isHttp10OrEarlier, boolean allowDuplicateContentLengths) {
            ...
            if (allowDuplicateContentLengths) {
                ...
                for (CharSequence field : contentLengthFields) {
                    ...
                    for (String token : tokens = field.toString().split(COMMA_STRING, -1)) {
                        ...
                        if (trimmed.equals((Object)firstValue)) continue;
        ...
        if (firstField.isEmpty() || !Character.isDigit((char)firstField.charAt(0))) {
            throw new IllegalArgumentException("Content-Length value is not a number: " + firstField);
        }
        try {
            long value = Long.parseLong((String)firstField);
            return ObjectUtil.checkPositiveOrZero((long)value, (String)"Content-Length value");
        }
        catch (NumberFormatException e) {
            throw new IllegalArgumentException("Content-Length value is not a number: " + firstField, (Throwable)e);
        }
    }
}
