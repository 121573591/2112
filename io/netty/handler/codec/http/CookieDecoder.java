...
public final class CookieDecoder {
    private final InternalLogger logger = InternalLoggerFactory.getInstance((Class)this.getClass());
    ...
    private Set<Cookie> doDecode(String header) {
        ...
        CookieDecoder.extractKeyValuePairs(header, (List<String>)names, (List<String>)values);
        if (names.isEmpty()) {
        ...
        if (((String)names.get(0)).equalsIgnoreCase(VERSION)) {
            try {
                version = Integer.parseInt((String)((String)values.get(0)));
            }
        ...
        while (i < names.size()) {
            ...
            while (j < names.size()) {
                ...
                if (DISCARD.equalsIgnoreCase(name)) {
                    ...
                    maxAge = Integer.parseInt((String)value);
                    ...
                    for (String s1 : portList = value.split(",")) {
                        try {
                            ports.add((Object)Integer.valueOf((String)s1));
                        }
                        catch (NumberFormatException numberFormatException) {
                            // empty catch block
                        }
            ...
            if (version > 1) {
                ...
                c.setPorts((Iterable<Integer>)ports);
                c.setDiscard(discard);
            }
            cookies.add((Object)c);
            ++i;
        }
        return cookies;
    }
    ...
    private DefaultCookie initCookie(String name, String value) {
        ...
        if (this.strict && (invalidOctetPos = CookieUtil.firstInvalidCookieNameOctet(name)) >= 0) {
            if (this.logger.isDebugEnabled()) {
                this.logger.debug("Skipping cookie because name '{}' contains invalid char '{}'", (Object)name, (Object)Character.valueOf((char)name.charAt(invalidOctetPos)));
            }
            return null;
        }
        ...
        if (this.strict && (invalidOctetPos = CookieUtil.firstInvalidCookieValueOctet(unwrappedValue)) >= 0) {
            if (this.logger.isDebugEnabled()) {
                this.logger.debug("Skipping cookie because value '{}' contains invalid char '{}'", (Object)unwrappedValue, (Object)Character.valueOf((char)unwrappedValue.charAt(invalidOctetPos)));
            }
            return null;
        }
