...
ISSLChannel {
    ...
    public SSLSocketChannel(SocketChannel inputSocketChannel, SSLEngine inputEngine, ExecutorService inputExecutor, SelectionKey key) throws IOException {
        ...
        this.myNetData = ByteBuffer.allocate((int)this.engine.getSession().getPacketBufferSize());
        this.peerNetData = ByteBuffer.allocate((int)this.engine.getSession().getPacketBufferSize());
    ...
    private boolean doHandshake() throws IOException {
        ...
        this.myAppData = ByteBuffer.allocate((int)appBufferSize);
        this.peerAppData = ByteBuffer.allocate((int)appBufferSize);
        this.myNetData.clear();
    ...
    private ByteBuffer enlargeBuffer(ByteBuffer buffer, int sessionProposedCapacity) {
        buffer = sessionProposedCapacity > buffer.capacity() ? ByteBuffer.allocate((int)sessionProposedCapacity) : ByteBuffer.allocate((int)(buffer.capacity() * 2));
        return buffer;
    }
