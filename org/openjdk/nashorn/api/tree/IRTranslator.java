...
extends SimpleNodeVisitor {
    ...
    public boolean enterBlockStatement(BlockStatement blockStatement) {
        ...
        if (blockStatement.isSynthetic()) {
            ...
            this.curStat = this.translateStat((Statement)block.getStatements().get(0));
        } else {
    ...
    public boolean enterLiteralNode(LiteralNode<?> literalNode) {
        ...
        if (value instanceof Lexer.RegexToken) {
            ...
            ArrayList exprTrees = new ArrayList(exprNodes.size());
            for (Node node : exprNodes) {
                ...
                exprTrees.add((Object)this.curExpr);
            }
            this.curExpr = new ArrayLiteralTreeImpl(literalNode, (List<? extends ExpressionTree>)exprTrees);
        } else {
    ...
    public boolean enterSwitchNode(SwitchNode switchNode) {
        ...
        ArrayList caseTrees = new ArrayList(caseNodes.size());
        for (CaseNode caseNode : caseNodes) {
            Block body = caseNode.getBody();
            caseTrees.add((Object)new CaseTreeImpl(caseNode, this.translateExpr(caseNode.getTest()), this.translateStats(body != null ? body.getStatements() : null)));
        }
        this.curStat = new SwitchTreeImpl(switchNode, this.translateExpr(switchNode.getExpression()), (List<? extends CaseTree>)caseTrees);
        return false;
    }
    ...
    public boolean enterTryNode(TryNode tryNode) {
        ...
        ArrayList catchTrees = new ArrayList(catchNodes.size());
        for (CatchNode catchNode : catchNodes) {
            catchTrees.add((Object)new CatchTreeImpl(catchNode, this.translateExpr(catchNode.getException()), (BlockTree)this.translateBlock(catchNode.getBody()), this.translateExpr(catchNode.getExceptionCondition())));
        }
        this.curStat = new TryTreeImpl(tryNode, (BlockTree)this.translateBlock(tryNode.getBody()), (List<? extends CatchTree>)catchTrees, (BlockTree)this.translateBlock(tryNode.getFinallyBody()));
        return false;
    }
    ...
    private boolean handleBlock(Block block, boolean sortStats) {
        ...
        this.curStat = new BlockTreeImpl(block, this.translateStats((List<? extends Statement>)(sortStats ? this.getOrderedStatements(stats) : stats)));
        return false;
    }

    private List<? extends Statement> getOrderedStatements(List<? extends Statement> stats) {
        ArrayList statList = new ArrayList(stats);
    ...
    private List<? extends StatementTree> translateStats(List<? extends Statement> stats) {
        ...
        ArrayList statTrees = new ArrayList(stats.size());
        for (Statement stat : stats) {
            this.curStat = null;
            stat.accept(this);
            assert (this.curStat != null);
            statTrees.add((Object)this.curStat);
        }
        return statTrees;
    }
    ...
    private List<? extends ExpressionTree> translateParameters(FunctionNode func) {
        ...
        if (paramExprs != null) {
            ...
            ArrayList exprTrees = new ArrayList(params.size());
            for (IdentNode ident : params) {
                IdentNode expr = paramExprs.containsKey((Object)ident) ? (Expression)paramExprs.get((Object)ident) : ident;
                this.curExpr = null;
    ...
    private List<? extends ExpressionTree> translateExprs(List<? extends Expression> exprs) {
        ...
        ArrayList exprTrees = new ArrayList(exprs.size());
    ...
    private List<? extends PropertyTree> translateProperties(List<PropertyNode> propNodes) {
        ArrayList propTrees = new ArrayList(propNodes.size());
        for (PropertyNode propNode : propNodes) {
            propTrees.add((Object)this.translateProperty(propNode));
        }
        return propTrees;
    }
