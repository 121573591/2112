...
public final class NameCodec {
    ...
    public static String encode(String name) {
        ...
        assert (name.equals((Object)NameCodec.decode(bn))) : name;
        return bn;
    }
    ...
    public static String decode(String name) {
        ...
        if (!sn.isEmpty() && NameCodec.looksMangled(name)) {
            ...
            assert (name.equals((Object)NameCodec.mangle(sn))) : name + " => " + sn + " => " + NameCodec.mangle(sn);
        }
        return sn;
    }
        ...
        for (int i = stringStart; i < slen; ++i) {
            ...
            if (c == '\\' && i + 1 < slen && (oc = NameCodec.originalOfReplacement(rc = s.charAt(i + 1))) != rc) {
                if (sb == null) {
                    ...
                    sb.append((CharSequence)s, stringStart, i);
                }
                ++i;
                c = oc;
            }
    ...
    private static char replacementOf(char c) {
        ...
        int i = DANGEROUS_CHARS.indexOf((int)c);
        if (i < 0) {
            return c;
        }
    ...
    private static char originalOfReplacement(char c) {
        ...
        int i = REPLACEMENT_CHARS.indexOf((int)c);
        if (i < 0) {
            return c;
        }
    ...
    private static boolean isDangerous(char c) {
        ...
        return DANGEROUS_CHARS.indexOf((int)c) >= 1;
    }
