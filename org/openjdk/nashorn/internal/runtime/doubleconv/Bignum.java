...
class Bignum {
    ...
    void addBignum(Bignum other) {
        ...
        this.align(other);
        this.ensureCapacity(1 + Math.max((int)this.bigitLength(), (int)other.bigitLength()) - this.exponent_);
        int carry = 0;
        ...
        this.used_digits_ = Math.max((int)bigit_pos, (int)this.used_digits_);
        assert (this.isClamped());
    }
    ...
    char divideModuloIntBignum(Bignum other) {
        ...
        if (other.used_digits_ == 1) {
            int quotient = Integer.divideUnsigned((int)this_bigit, (int)other_bigit);
            this.bigits_[this.used_digits_ - 1] = this_bigit - other_bigit * quotient;
            assert (Integer.compareUnsigned((int)quotient, (int)65536) < 0);
            result = (char)(result + quotient);
            this.clamp();
            return result;
        }
        int division_estimate = Integer.divideUnsigned((int)this_bigit, (int)(other_bigit + 1));
        assert (Integer.compareUnsigned((int)division_estimate, (int)65536) < 0);
    ...
    static int compare(Bignum a, Bignum b) {
        ...
        for (int i = bigit_length_a - 1; i >= Math.min((int)a.exponent_, (int)b.exponent_); --i) {
            int bigit_b;
    ...
    static int plusCompare(Bignum a, Bignum b, Bignum c) {
        ...
        int borrow = 0;
        int min_exponent = Math.min((int)Math.min((int)a.exponent_, (int)b.exponent_), (int)c.exponent_);
    ...
    public String toString() {
        return "Bignum" + Arrays.toString((int[])this.bigits_);
    }
}
