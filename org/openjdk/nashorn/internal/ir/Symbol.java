...
Serializable {
    ...
    private static String align(String string, int max) {
        ...
        sb.append((CharSequence)string, 0, Math.min((int)string.length(), (int)max));
        while (sb.length() < max) {
            sb.append(' ');
        }
        return sb.toString();
    ...
    private boolean shouldTrace() {
        return TRACE_SYMBOLS != null && (TRACE_SYMBOLS.isEmpty() || TRACE_SYMBOLS.contains((Object)this.name));
    }

    private void trace(String desc) {
        Context.err(Debug.id(this) + " SYMBOL: '" + this.name + "' " + desc);
        if (TRACE_SYMBOLS_STACKTRACE != null && (TRACE_SYMBOLS_STACKTRACE.isEmpty() || TRACE_SYMBOLS_STACKTRACE.contains((Object)this.name))) {
    ...
    static {
        ...
        if (stacktrace != null) {
            ...
            while (st.hasMoreTokens()) {
                TRACE_SYMBOLS_STACKTRACE.add((Object)st.nextToken());
            }
        } else {
        ...
        if (trace != null) {
            ...
            while (st.hasMoreTokens()) {
                TRACE_SYMBOLS.add((Object)st.nextToken());
            }
        } else {
            TRACE_SYMBOLS = null;
        }
