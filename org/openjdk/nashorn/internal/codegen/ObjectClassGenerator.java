...
implements Loggable {
    ...
    public DebugLogger initLogger(Context ctxt) {
        return ctxt.getLogger((Class<? extends Loggable>)this.getClass());
    }
    ...
    public static long pack(Number n) {
        ...
        if (n instanceof Double) {
            return Double.doubleToRawLongBits((double)n.doubleValue());
        }
    ...
    public static int getFieldCount(Class<?> clazz) {
        ...
        if (prefix.equals((Object)name)) {
            return 0;
        }
        int scopeMarker = name.indexOf(SCOPE_MARKER);
        return Integer.parseInt((String)(scopeMarker == -1 ? name.substring(prefix.length()) : name.substring(prefix.length(), scopeMarker)));
    }
    ...
    public byte[] generate(String descriptor) {
        ...
        int fieldCount = Integer.valueOf((String)counts[0]);
        if (counts.length == 1) {
            return this.generate(fieldCount);
        }
        int paramCount = Integer.valueOf((String)counts[1]);
    ...
    private List<String> addFields(ClassEmitter classEmitter, int fieldCount) {
        LinkedList initFields = new LinkedList();
        ...
        for (int i = 0; i < fieldCount; ++i) {
            for (Type type : fieldTypes) {
                ...
                initFields.add((Object)fieldName);
            }
        }
        return initFields;
    }
    ...
    private static void newAllocate(String className, ClassEmitter classEmitter) {
        MethodEmitter allocate = classEmitter.method((EnumSet<ClassEmitter.Flag>)EnumSet.of((Enum)ClassEmitter.Flag.PUBLIC, (Enum)ClassEmitter.Flag.STATIC), CompilerConstants.ALLOCATE.symbolName(), ScriptObject.class, PropertyMap.class);
        allocate.begin();
    ...
    private static MethodHandle createGetterInner(Class<?> forType, Class<?> type, MethodHandle primitiveGetter, MethodHandle objectGetter, List<MethodHandle> converters, int programPoint) {
        ...
        if (forType == null) {
            if (isOptimistic) {
                if (ti == 2) {
                    return Lookup.MH.dropArguments((MethodHandle)JSType.GET_UNDEFINED.get(2), 0, Object.class);
                }
            ...
            return Lookup.MH.dropArguments((MethodHandle)JSType.GET_UNDEFINED.get(ti), 0, Object.class);
        }
        ...
        if (isOptimistic) {
            ...
            assert (fti > ti);
            if (fti == 2) {
                return Lookup.MH.filterReturnValue(objectGetter, Lookup.MH.insertArguments((MethodHandle)converters.get(ti), 1, programPoint));
            }
        ...
        if (fti == 2) {
            if (fti != ti) {
                return Lookup.MH.filterReturnValue(tgetter, (MethodHandle)JSType.CONVERT_OBJECT.get(ti));
            }
            return tgetter;
        }
