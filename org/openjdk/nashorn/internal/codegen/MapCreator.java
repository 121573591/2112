...
public class MapCreator<T> {
    ...
    PropertyMap makeFieldMap(boolean hasArguments, boolean dualFields, int fieldCount, int fieldMaximum, boolean evalCode) {
        ...
        return PropertyMap.newMap((Collection<Property>)properties, this.structure.getName(), fieldCount, fieldMaximum, 0);
    }
    ...
    PropertyMap makeSpillMap(boolean hasArguments, boolean dualFields) {
        ...
        for (MapTuple tuple : this.tuples) {
            ...
            properties.add((Object)new SpillProperty(key, flags, spillIndex++, initialType));
        }
        return PropertyMap.newMap((Collection<Property>)properties, this.structure.getName(), 0, 0, spillIndex);
    }
