...
public class Conversion {
    ...
    public static char binaryBeMsb0ToHexDigit(boolean[] src, int srcPos) {
        if (Integer.compareUnsigned((int)srcPos, (int)src.length) >= 0) {
            if (src.length == 0) {
    ...
    public static int hexDigitToInt(char hexDigit) {
        int digit = Character.digit((char)hexDigit, (int)16);
        if (digit < 0) {
            throw new IllegalArgumentException("Cannot interpret '" + hexDigit + "' as a hexadecimal digit");
        }
        return digit;
    ...
    public static char intToHexDigit(int nibble) {
        char c = Character.forDigit((int)nibble, (int)16);
        if (c == '\u0000') {
    ...
    public static byte[] uuidToByteArray(UUID src, byte[] dst, int dstPos, int nBytes) {
        ...
        Conversion.longToByteArray(src.getMostSignificantBits(), 0, dst, dstPos, Math.min((int)nBytes, (int)8));
        if (nBytes >= 8) {
