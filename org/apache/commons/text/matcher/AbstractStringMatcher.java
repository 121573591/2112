...
implements StringMatcher {
    ...
    extends AbstractStringMatcher {
        ...
        CharSetMatcher(char[] chars) {
            ...
            Arrays.sort((char[])this.chars);
        }

        @Override
        public int isMatch(char[] buffer, int start, int bufferStart, int bufferEnd) {
            return Arrays.binarySearch((char[])this.chars, (char)buffer[start]) >= 0 ? 1 : 0;
        }

        @Override
        public int isMatch(CharSequence buffer, int start, int bufferStart, int bufferEnd) {
            return Arrays.binarySearch((char[])this.chars, (char)buffer.charAt(start)) >= 0 ? 1 : 0;
        }
        ...
        public String toString() {
            return super.toString() + Arrays.toString((char[])this.chars);
        }
    }
    ...
    extends AbstractStringMatcher {
        ...
        CharArrayMatcher(char ... chars) {
            this.string = String.valueOf((char[])chars);
            this.chars = (char[])chars.clone();
        }
