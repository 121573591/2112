...
public class BeanUtil {
    ...
    public static String okNameForRegularGetter(AnnotatedMethod am, String name, boolean stdNaming) {
        if (name.startsWith("get")) {
            if ("getCallbacks".equals((Object)name) ? BeanUtil.isCglibGetCallbacks(am) : "getMetaClass".equals((Object)name) && BeanUtil.isGroovyMetaClassGetter(am)) {
                return null;
            }
    ...
    protected static boolean isCglibGetCallbacks(AnnotatedMethod am) {
        ...
        if (rt.isArray() && (className = (compType = rt.getComponentType()).getName()).contains((CharSequence)".cglib")) {
    ...
    protected static String legacyManglePropertyName(String basename, int offset) {
        ...
        if (c == (d = Character.toLowerCase((char)c))) {
    ...
    public static String stdManglePropertyName(String basename, int offset) {
        ...
        if (c0 == (c1 = Character.toLowerCase((char)c0))) {
            return basename.substring(offset);
        }
        if (offset + 1 < end && Character.isUpperCase((char)basename.charAt(offset + 1))) {
            return basename.substring(offset);
        }
        StringBuilder sb = new StringBuilder(end - offset);
        sb.append(c1);
    ...
    public static String checkUnsupportedType(JavaType type) {
        ...
        return String.format((String)"%s type %s not supported by default: add Module \"%s\" to enable handling", (Object[])new Object[]{typeName, ClassUtil.getTypeDescription(type), moduleName});
    }
