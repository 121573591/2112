...
public class EnumNamingStrategies {
    ...
    implements EnumNamingStrategy {
        ...
        private static int indexIn(CharSequence sequence2, int start) {
            int length = sequence2.length();
            for (int i = start; i < length; ++i) {
                if ('_' != sequence2.charAt(i)) continue;
                return i;
            }
            return -1;
        }
        ...
        private static char charToUpperCaseIfLower(char c) {
            return Character.isLowerCase((char)c) ? Character.toUpperCase((char)c) : c;
        }

        private static char charToLowerCaseIfUpper(char c) {
            return Character.isUpperCase((char)c) ? Character.toLowerCase((char)c) : c;
        }
    }
}
