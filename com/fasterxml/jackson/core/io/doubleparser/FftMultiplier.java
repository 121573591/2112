...
class FftMultiplier {
    public static final double COS_0_25 = Math.cos((double)0.7853981633974483);
    public static final double SIN_0_25 = Math.sin((double)0.7853981633974483);
    ...
    private static ComplexVector calculateRootsOfUnity(int n) {
        ...
        for (int i = 1; i < n / 2; ++i) {
            ...
            cos = Math.cos((double)angle);
            sin = Math.sin((double)angle);
    ...
    private static void fft(ComplexVector a, ComplexVector[] roots2) {
        int s;
        int n = a.length;
        int logN = 31 - Integer.numberOfLeadingZeros((int)n);
    ...
    private static void fft3(ComplexVector a0, ComplexVector a1, ComplexVector a2, int sign, double scale) {
        double omegaImag = (double)sign * -0.5 * Math.sqrt((double)3.0);
    ...
    static BigInteger fromFftVector(ComplexVector fftVec, int signum, int bitsPerFftPoint) {
        ...
        int fftLen = (int)Math.min((long)fftVec.length, (long)(0x80000000L / (long)bitsPerFftPoint + 1L));
        ...
        int magComponent = 0;
        int prevIdx = Math.min((int)Math.max((int)0, (int)(bitIdx >> 3)), (int)(mag.length - 4));
        for (int part = 0; part <= 1; ++part) {
            for (int fftIdx = 0; fftIdx < fftLen; ++fftIdx) {
                long fftElem = Math.round((double)fftVec.part(fftIdx, part)) + carry;
                carry = fftElem >> bitsPerFftPoint;
                int idx = Math.min((int)Math.max((int)0, (int)(bitIdx >> 3)), (int)(mag.length - 4));
    ...
    static BigInteger multiplyFft(BigInteger a, BigInteger b) {
        ...
        int bitLen = Math.max((int)aMag.length, (int)(bMag = (b.signum() < 0 ? b.negate() : b).toByteArray()).length) * 8;
        int fftLen = (bitLen + (bitsPerPoint = FftMultiplier.bitsPerFftPoint(bitLen)) - 1) / bitsPerPoint + 1;
        if (fftLen < (fftLen3 = (fftLen2 = 1 << (logFFTLen = 32 - Integer.numberOfLeadingZeros((int)(fftLen - 1)))) * 3 / 4) && logFFTLen > 3) {
    ...
    static BigInteger squareFft(BigInteger a) {
        ...
        int fftLen = (bitLen + (bitsPerPoint = FftMultiplier.bitsPerFftPoint(bitLen)) - 1) / bitsPerPoint + 1;
        if (fftLen < (fftLen3 = (fftLen2 = 1 << (logFFTLen = 32 - Integer.numberOfLeadingZeros((int)(fftLen - 1)))) * 3 / 4)) {
            fftLen = fftLen3;
    ...
    static ComplexVector toFftVector(byte[] mag, int fftLen, int bitsPerFftPoint) {
        ...
        if (mag.length < 4) {
            ...
            System.arraycopy((Object)mag, (int)0, (Object)paddedMag, (int)(4 - mag.length), (int)mag.length);
            mag = paddedMag;
        }
        int base = 1 << bitsPerFftPoint;
        int halfBase = base / 2;
