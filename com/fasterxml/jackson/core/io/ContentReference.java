...
implements Serializable {
    ...
    protected String _truncate(CharSequence cs, int[] offsets, int maxSnippetLen) {
        ...
        int start = offsets[0];
        int length = Math.min((int)offsets[1], (int)maxSnippetLen);
    ...
    protected int _append(StringBuilder sb, String content) {
        ...
        for (int i = 0; i < end; ++i) {
            ...
            if (Character.isISOControl((char)ch) && this._appendEscaped(sb, ch)) continue;
            sb.append(ch);
        }
        sb.append('\"');
    ...
    public int hashCode() {
        return Objects.hashCode((Object)this._rawContent);
    }
}
