...
lbl28:
    ...
    protected int convertSurrogate(int secondPart) throws IOException {
        ...
        if (secondPart < 56320 || secondPart > 57343) {
            throw new IOException("Broken surrogate pair: first char 0x" + Integer.toHexString((int)firstPart) + ", second 0x" + Integer.toHexString((int)secondPart) + "; illegal combination");
        }
    ...
    protected static String illegalSurrogateDesc(int code) {
        if (code > 0x10FFFF) {
            return "Illegal character point (0x" + Integer.toHexString((int)code) + ") to output; max is 0x10FFFF as per RFC 4627";
        }
        if (code >= 55296) {
            if (code <= 56319) {
                return "Unmatched first part of surrogate pair (0x" + Integer.toHexString((int)code) + ")";
            }
            return "Unmatched second part of surrogate pair (0x" + Integer.toHexString((int)code) + ")";
        }
        return "Illegal character point (0x" + Integer.toHexString((int)code) + ") to output";
    }
}
