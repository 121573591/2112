...
extends ByteString {
    ...
    private RopeByteString(ByteString left, ByteString right) {
        ...
        this.treeDepth = Math.max((int)left.getTreeDepth(), (int)right.getTreeDepth()) + 1;
    }
    ...
    static ByteString concatenate(ByteString left, ByteString right) {
        ...
        if (newLength >= RopeByteString.minLength(newDepth = Math.max((int)left.getTreeDepth(), (int)right.getTreeDepth()) + 1)) {
    ...
    public ByteBuffer asReadOnlyByteBuffer() {
        ByteBuffer byteBuffer = ByteBuffer.wrap((byte[])this.toByteArray());
    ...
    public List<ByteBuffer> asReadOnlyByteBufferList() {
        ...
        while (pieces.hasNext()) {
            ...
            result.add((Object)byteString.asReadOnlyByteBuffer());
        }
        return result;
    }
    ...
    private boolean equalsFragments(ByteString other) {
        ...
        while (true) {
            ...
            int bytesToCompare = Math.min((int)thisRemaining, (int)thatRemaining);
    ...
    extends InputStream {
        ...
        private int readSkipInternal(byte[] b, int offset, int length) {
            ...
            for (bytesRemaining = length; bytesRemaining > 0; bytesRemaining -= count) {
                ...
                count = Math.min((int)currentPieceRemaining, (int)bytesRemaining);
                if (b != null) {
    ...
    implements Iterator<ByteString.LeafByteString> {
        ...
        private PieceIterator(ByteString root) {
            if (root instanceof RopeByteString) {
                ...
                this.breadCrumbs.push((Object)rbs);
        ...
        private ByteString.LeafByteString getNextNonEmptyLeaf() {
            ...
            do {
                ...
                return null;
            } while ((result = this.getLeafByLeft(((RopeByteString)this.breadCrumbs.pop()).right)).isEmpty());
            return result;
        }
    ...
    private static class Balancer {
        ...
        private void insert(ByteString byteString) {
            ...
            if (this.prefixesStack.isEmpty() || ((ByteString)this.prefixesStack.peek()).size() >= binEnd) {
                this.prefixesStack.push((Object)byteString);
            } else {
                ByteString left;
                ...
                while (!this.prefixesStack.isEmpty()) {
                    ...
                    if (((ByteString)this.prefixesStack.peek()).size() >= binEnd) break;
                    left = (ByteString)this.prefixesStack.pop();
                    newTree = new RopeByteString(left, newTree);
                }
                this.prefixesStack.push((Object)newTree);
            }
        }

        private int getDepthBinForLength(int length) {
            int depth = Arrays.binarySearch((int[])minLengthByDepth, (int)length);
            if (depth < 0) {
